...
- [x] **06/08/2026**: **Trading FCP — 2 fixes de UX en la ventana "Slot 6 — Análisis en progreso"** (`Trading_FCP.py`). (1) Los botones "Sí, ejecutar / No, cerrar" (que lanzan el análisis contextual de Claude) quedaban **fuera del área visible**: el log con `expand=True` ocupaba todo y el `frame_accion` se empaquetaba al final, debajo → había que agrandar la ventana. Fix: el panel se ancla `side="bottom"` desde el inicio + el Text del log pide `height=12` mínimo + ventana 500→560. (2) La **ventana CMD** del análisis contextual **se cerraba al terminar** (se lanzaba con `python ... CREATE_NEW_CONSOLE`; al salir Python, la consola muere). Fix: lanzar con `cmd /k` (igual que el lanzador `.ps1`), con doble comilla externa para rutas con espacios → la ventana con los resultados **queda abierta**. Probados (layout con Tk oculto: panel en y=476..560; quoting con `cmd /c` + script temporal).
- [x] **07/08/2026**: **Confirmada la Parte B del capital LIVE** (`sync_ibkr_flex.py`). El usuario editó su Activity Flex Query en IBKR agregando **Position Value** (Open Positions) y la sección **Equity Summary in Base**. Verificado en el sync de 08:58: `ultimo_sync_real` ahora trae `stocks_por_moneda` (USD 7,789.94) y la línea "Capital:" de LIVE muestra el total desglosado como PAPER: `£11,492.65 = $7,789.94 + £201.25 + $7,401.17`. Sin cambios de código (la Parte B ya estaba; solo faltaban los campos del Flex). Aparte: `trigger_analisis_claude.json` se escribe sin BOM (el BOM hacía que el hook `check_slot6_trigger` reportara "PENDIENTE" aunque estuviera `confirmado`).
- [x] **21/08/2026**: **Fallo del Slot 6 por token OAuth vencido + auto-recuperación.** La corrida programada (08:01) y un reintento manual por la interfaz dejaron **8 borradores duplicados, todos SIN aprobar**: la revisión `claude -p --solo-revision` rebotó con `401 OAuth access token has expired`. **Diagnóstico** (con el git log del día como evidencia — 2 corridas mecánicas 08:02-08:11 y 08:46-08:50, y el fallo de la 2ª en ~1 min prueba que la espera no se activó): el pre-chequeo por `claude auth status` **no detecta** un token vencido server-side (reporta `loggedIn:true` igual), por eso `esperar_reautenticacion()` nunca se disparaba; y la renovación OAuth es **interactiva** (en la corrida desatendida de las 8 AM nadie la hace). **Fixes**: (1) `run_slot6_cmd.py` — la revisión corre en un **LOOP** que captura el 401 **durante** la corrida (patrones ampliados con "oauth access token has expired"/401), espera la reautenticación hasta 10 min y **reintenta solo**; si no se completa, deja `data/alerta_reauth_slot6.json`. (2) `Recomendar_Compra_Venta.py` — la GUI lee esa alerta al abrir y avisa (visible aunque nadie haya visto el CMD). **Slot 6 del día completado a mano** con el token ya válido: revisado y aprobado (claude-opus-4-8), vetos **PLTR ×2** (RSI 78.2, sobrecompra) y **OXY** (máximos + resistencia) en contexto cauto (Nasdaq −1%, VIX↑, Fed hawkish, Irán); **4 duplicados purgados**. **Límite conocido**: OAuth no se puede renovar solo; la corrida de las 8 AM sigue necesitando `claude auth login` en algún momento.
- [x] **24/08/2026**: `sync_ibkr_flex.py` v1.6.0 — **reintento con backoff ante DNS/conexión transitoria**. El sync cortaba al primer fallo de DNS (`[Errno 11002] getaddrinfo failed`), que resultó **intermitente** (resolvía y fallaba en segundos). Nuevo `_get_con_reintentos` envuelve los `requests.get` (SendRequest y GetStatement): ante `ConnectionError`/`Timeout` reintenta hasta 4× con backoff (15/30/45s) y solo se rinde si el fallo es persistente. Probado con mock (recupera al 3er intento; re-lanza tras agotar).
- [x] **25/08/2026**: **Duplicados manuales IBKR-UK Real** eliminados: AVGO (compra manual 08-20 @363) e IGLN.L (venta manual 08-24 @90.195), ambos duplicaban fills del sync. El de AVGO había **reaparecido tras un `git pull`** (el borrado desde la GUI era local y sin commitear). Post-fix: AVGO=1, IGLN.L=51 (cuadran con IBKR).
- [x] **25/08/2026**: **GUI — commit+push automático del historial** (`Recomendar_Compra_Venta.py` v3.12.0). Causa raíz del punto anterior: los borrados/cargas manuales se guardaban solo en el archivo local y un `git pull`/sync posterior los revertía. Nuevo `commitear_historial_git()` (add + commit + `pull --rebase --autostash` + push de `historial_operaciones.json`), llamado tras **eliminar / registrar / editar** operaciones. Silencioso ante errores (no rompe la GUI).
- [x] **29/08/2026**: **Reconciliación PLTR IBKR-UK Paper** — el sync por TWS API (`ib_insync`) solo trae ejecuciones **del día**, así que las 2 ventas del 08-19 (@173.52, @173.05) nunca se capturaron y el historial mostraba +2. Agregadas manualmente (con `exec_id` en formato Flex para que un futuro sync las deduplique) → PLTR=3=TWS.
- [x] **29/08/2026**: **Sync Flex para Paper** (`sync_ibkr_flex.py` v1.7.0 → v1.7.1). Parametrizado por **MODO**: `--paper` sincroniza la cuenta Paper (`config_flex_ibkr_paper.json` → `ultimo_sync_paper`, ops `modo=Paper`, alerta en `alertas_discrepancias_paper.json`); sin el flag, Real como siempre. **Motivo**: el sync por TWS API pierde trades de días no sincronizados; el Flex trae un **rango de 30 días** y no se pierden. Fix v1.7.1: aceptar `OpenPosition` **sin** `assetCategory` (el Flex de Paper no incluye ese campo → antes descartaba todas las posiciones). GUI v3.12.1 lee `alertas_discrepancias_paper.json`. **Bonus (bug latente):** el `.gitignore` estaba en **UTF-16** y git **no lo leía** (ninguna regla funcionaba → riesgo de commitear el token Flex); reescrito en UTF-8 + reglas de Paper. Nuevos: `config_flex_ibkr_paper_template.json`, `sync_ibkr_flex_paper.bat`. Verificado end-to-end contra el reporte real: 8 posiciones + 47 trades + capital total (`£1,027,251 = $stocks + £cash + $cash`); historial Paper cuadra con el Flex.
- [x] **29/08/2026**: **Tarea programada del Flex Paper** — creada `Sync IBKR Flex Paper` (L-V **08:05**, StartWhenAvailable + WakeToRun, corre `sync_ibkr_flex_paper.bat`) y **movido el análisis Slot 6** (`Trigger_Slot6_NY`) de 08:01 a **08:07** (lo cambió el usuario; la tarea está protegida y no se puede editar sin admin). Cronología final de días hábiles: **07:58 Real → 08:05 Paper → 08:07 análisis** (usa posiciones frescas de ambas cuentas).

---

## Tareas Completadas - Septiembre 2026

- [x] **15/09/2026**: **Fix dedup por zona horaria + limpieza META Paper** (`sync_ibkr_flex.py` v1.9.1). El sync Paper marcó META IBKR=2 vs Historial=1: la venta de 1 META @656.7 del 14-09 quedó **duplicada** (misma operación capturada por `sync_flex` y `sync_ibkr`). El dedup por firma (v1.8.x) NO la cazó porque comparaba el campo **`hora`**, que las dos fuentes guardan en **zonas horarias distintas** (sync_flex en ET `09:30`, sync_ibkr en UTC `13:30`) → las veía a ~4h de diferencia (> tolerancia 300s) y las trataba como fills distintos. **Fix**: `_segundos()` ahora toma el timestamp del **`exec_id`** (HHMMSS en UTC, consistente entre fuentes; ambos exec eran `133000` vs `133010` = 10s), con fallback a `hora` si el exec_id no trae timestamp. Es también la causa de que MSFT 09-09 se hubiera escapado. Eliminado el duplicado `sync_ibkr` → los 10 tickers Paper cuadran. Tests OK (caso real de zona horaria + regresiones).
- [x] **14/09/2026**: **Fix de fondo del flujo de git — FASE 2 (migración del resto de puntos).** Migrados a `git_utils.commit_pull_push` (lock compartido + pull --rebase --autostash + reintento): `Recomendar_Compra_Venta.subir_estado_ibkr_a_github` (v3.15.0), `sync_ibkr_automatico.subir_a_github` (v1.4.0), y `Trading_Claude._push` (v2.10.0, con `git add -u`). El hook `check_slot6_trigger.py:git_pull` ahora hace el pull **bajo el lock** (timeout 30s, omite si no lo consigue; fallback al comportamiento previo si git_utils no importa). **Hallazgo (rule 11):** `sincronizar_desde_github`/`sincronizar_github_headless` (en Recomendar, DESCARGAR_DATA y automatizar_trading) hacen **solo `git fetch` + `git show`** (read-only, el merge es en Python) → NO causan rebases atascados ni pérdidas, así que NO necesitan lock; se dejaron sin tocar. `ejecutar_slot6_todas_plataformas` solo limpia merges atascados (no commitea/pushea) → sin cambios. Con esto, TODOS los puntos que commitean/pushean quedan serializados por el lock. Commiteado end-to-end con el propio `commit_pull_push`.
- [x] **11/09/2026**: **Fix de fondo del flujo de git — FASE 1 (lock compartido).** Causa raíz de las pérdidas recurrentes (venta TSLA 03‑09, compra GOOGL Real, ops TYBA 09‑09, feature "Todos" atrapada en autostash, error de push en `trigger_slot6_ny.ps1`): varios procesos tocaban git EN PARALELO → rebases atascados/reverts/autostash colgado. **Nuevo `git_utils.py` v1.0.0**: `git_lock(timeout)` (lock advisory por archivo `.git/trading_git_lock`, roba locks stale de procesos muertos) + `commit_pull_push(archivos, mensaje)` (bajo el lock: limpia rebase/merge colgado → add → commit → loop `pull --rebase --autostash` + push con **reintento** → devuelve ok=True SOLO si el push realmente entró). **Migrados 3 puntos** (los que más chocan a la mañana): `sync_ibkr_flex.git_commit_push` (v1.9.0), `Recomendar_Compra_Venta.commitear_historial_git` (v3.14.0, sigue en hilo para no congelar la GUI), y `trigger_slot6_ny.ps1` (lock + pull-before-push + reintento en PowerShell, mismo lock file que el Python). Tests OK: exclusión mutua entre hilos, robo de lock stale, y caso "nada para commitear". Falta la Fase 2 (migrar el resto de puntos de git) — ver Pendientes en CLAUDE.md.
- [x] **10/09/2026**: **Filtro "Todos" en Historial de Operaciones** (`Recomendar_Compra_Venta.py` v3.13.0). Nueva opción "Todos" en el combo Plataforma: agrega columna **Plataforma** a la tabla de cartera (desglose por plataforma+modo) y a la tabla de operaciones (mostrando **solo acciones en cartera**, ordenadas agrupadas por plataforma). Saldos FIFO calculados **por grupo** (una venta de una plataforma no consume compras de otra). "Capital y Posiciones" y "Resumen" se ocultan/blanquean con "Todos" (mezclar USD/GBP daría números erróneos). Validado: tenencias por grupo coinciden con el sync (IBKR-UK Paper 10/10, Real 3/3). Nota: durante el desarrollo, la feature sin commitear quedó atrapada en un autostash de un rebase colgado (otro caso del problema de git recurrente) — recuperada del autostash y commiteada.
- [x] **10/09/2026**: **Fix regresión del dedup: la firma no incluía plataforma+modo.** Al día siguiente de desplegar el dedup por firma (v1.8.0), el sync Real marcó un falso descuadre "GOOGL: IBKR=1, Historial=0". Causa: la firma de fill era `(ticker, tipo, cantidad, precio, día)` sin **plataforma/modo**, y como la lista de operaciones existentes mezcla Paper y Real, la compra de **GOOGL Real** se dedupeó por error contra la de **GOOGL Paper** del mismo día/precio (GOOGL se opera en ambas cuentas) → se omitió y se perdió. La duda del usuario ("¿por qué no salió en syncs anteriores?") fue la pista: el descuadre apareció exactamente el día después del deploy. **Fix** (`sync_ibkr_flex.py` v1.8.1): agregar `plataforma`+`modo` a la firma. 8 tests OK, incluido el nuevo caso "Real no se mezcla con Paper". Re-ejecutado el sync Real → recupera la compra de GOOGL Real y el descuadre desaparece.
- [x] **09/09/2026**: **Duplicado MSFT Paper + dedup del sync endurecido.** El sync Paper marcó MSFT IBKR=4 vs Historial=5: la compra de 1 MSFT @ $493.52 del 08-09 quedó registrada dos veces (`sync_ibkr` exec ...133012 y `sync_flex` exec ...133001, mismo fill con ~11s de diferencia) — 3ra vez del patrón (tras 31-ago). Eliminado el duplicado `sync_ibkr` (los 10 tickers Paper cuadran). **Fix de fondo**: `sync_ibkr_flex.py` v1.8.0 — extraído el dedup a `filtrar_operaciones_nuevas()` con un **3er nivel por FIRMA de fill** (ticker+tipo+cantidad+precio+día + tolerancia de tiempo de 300s), que caza el mismo fill capturado por dos fuentes (Flex vs TWS API) con `exec_id`/hora distintos y solo una con `ib_exec_id`. Es count-aware y **no colapsa fills múltiples legítimos** (si ambos traen `ib_exec_id` distinto, son fills reales distintos). 7 tests unitarios OK. Nota: `sync_ibkr_automatico.py` (TWS API) tiene un dedup similar solo-`exec_id`, pero como TWS solo captura fills del día en curso, el patrón siempre entra por el Flex; se deja como spot conocido.
...
- [x] **04/09/2026**: **Venta TSLA IBKR-UK Real perdida por push rechazado + enredo de git concurrente.** El usuario vendió 2 TSLA @ $370 en IBKR-UK Real (09-03), pero no aparecía en el historial pese a que "el sync corrió ok". **Diagnóstico**: el sync Flex de las 07:58 (commit `cfc5d02`) **sí capturó la venta** (con `exec_id` real `TSLA_20260903133137_SLD_2`, TSLA 2→0) y mostró la ventana verde de éxito legítima, pero su **`git push` fue rechazado** (`! [rejected] main`), y como corrían la GUI y el CMD del Slot 6 tocando git en paralelo, se armaron **dos líneas divergentes** desde `bb896ce`: la remota (`552ef1d` → `007e1fb`: precios + Slot 6 09-04 + ventas TYBA manuales) y la local (`cfc5d02`…`37f3b4b`: syncs 09-04 + venta TSLA @370 + AVGO @350 TYBA). Ninguna tenía todo (diferían en **5 operaciones**). **Solución**: reset de `main` a `007e1fb` + merge manual de las 2 ops faltantes (TSLA @370, AVGO @350) + refresco de `ultimo_sync_real` (09-04, TSLA fuera); IBKR-UK Real cuadra con IBKR (TSLA=0); pusheado a `a80df59`. Sin pérdida de datos (reflog + backup). **Bug de fondo pendiente**: el sync loguea `[WARN] Git falló` ante push rechazado pero termina como "OK" y no reintenta; combinado con git concurrente de la GUI/CMD, se pierde el commit del sync.
