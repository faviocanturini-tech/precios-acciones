...
def iniciar_proceso():
    ...
    if df['Fecha'].isna().any():
        ...
        if messagebox.askyesno("Fechas no convertidas", "Algunas filas no pudieron convertirse a fecha.\n¿Deseas continuar igual (esas filas quedarán con fecha vacía)?"):
    ...
    df["Rentabilidad"] = df.apply(lambda r: (r["Margen"] / r["Aporte acumulado"] * 100) if r["Aporte acumulado"] > 0 else 0, axis=1)
    ...
    tk.Label(frame1, fg="blue", text=f"Max % var : {max_var:.2f}% ({fecha_max_var})", font=("Arial", 12)).pack(anchor="w")
    tk.Label(frame1, fg="blue", text=f"Min % var : {min_var:.2f}% ({fecha_min_var})", font=("Arial", 12)).pack(anchor="w")
    ...
    tk.Label(frame3, fg="black", text=f"Acciones Vendidas      : {acciones_vendidas}", font=("Arial", 12)).pack(anchor="w")
    ...
    tk.Label(frame4, fg="purple", text=f"Rentab. max    : {max_rentab:.2f}% ({fecha_max_rentab})", font=("Arial", 12)).pack(anchor="w")
